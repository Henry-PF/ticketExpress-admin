...
export default function Update() {
  ...
  const handleSaveChange = async () => {
    try {
      const data = await axios.put('http://localhost:3001/buses/update_bus', selectedBus);
      if (data.status === 200) {
        Swal.fire({
    ...
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const { data } = axios.put(`http://localhost:3001/buses/delete_bus/${id}/2`);
          Swal.fire(
  ...
  const handleActive = async (id) => {
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const { data } = axios.put(`http://localhost:3001/buses/delete_bus/${id}/1`);
          Swal.fire(
            'Servicio Habilitado!',
